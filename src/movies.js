...
// const obj1 ={a:10, b: "Hellow"};
// const obj2 = {a:10, b:"Hellow"};
...
// console.log(httpPrefix + moviesData.results[0].backdrop_path);
// console.log(httpPrefix + moviesData.results[2]."movies-list"poster_path);
const ulElement = document.querySelector(".movies-list");
const sectionElements = document.querySelectorAll("section");
const mostPopularMovies = document.querySelector(".most-popular");
const leastPopularMovies  = document.querySelector(".least-popular");

ulElement.innerHTML = getPosterImages();

function getPosterImages(){
    const arImages = moviesData.results.map(movie=>`<li class ="movie- item">
    <img class ="movie-image"src="${httpPrefix}${movie.poster_path}">
    <div class="movie-details"><div class="movie -title">${movie.title}</div>
    <p class="movie- description">${movie.overview}<p></li>`);
